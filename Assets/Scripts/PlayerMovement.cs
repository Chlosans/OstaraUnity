...
namespace walkingPlayer
{
    public class PlayerMovement : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (move == "{\"data\":\"X-1Y-11\"}")
            {
                _direction.x -= 1;
                _direction.z -= 1;
            }

            if (move == "{\"data\":\"X1Y01\"}" || move == "{\"data\":\"X1Y01\"}")
            {
                _direction.x += 1;
            }

            if (move == "{\"data\":\"X0Y11\"}" || move == "{\"data\":\"X-0Y11\"}")
            {
                _direction.z += 1;
            }

            if (move == "{\"data\":\"X-1Y01\"}" || move == "{\"data\":\"X-1Y-01\"}")
            {
                _direction.x -= 1;
            }

            if (move == "{\"data\":\"X0Y-11\"}" || move == "{\"data\":\"X-0Y-11\"}")
            {
                _direction.z -= 1;
            }
