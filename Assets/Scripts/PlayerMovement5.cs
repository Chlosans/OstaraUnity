...
// réception mouvements/aniamtions/jumps du joueur 4
namespace walkingPlayer5
{
    public class PlayerMovement5 : MonoBehaviour
    {
        private AfficheButton5 _testJson;
        ...
        private void Update()
        {
            ...
            if (move == "{\"data\":\"X-1Y-14\"}")
            {
                _direction.x -= 1;
                _direction.z -= 1;
            }

            if (move == "{\"data\":\"X1Y04\"}" || move == "{\"data\":\"X1Y04\"}")
            {
                _direction.x += 1;
            }

            if (move == "{\"data\":\"X0Y14\"}" || move == "{\"data\":\"X-0Y14\"}")
            {
                _direction.z += 1;
            }

            if (move == "{\"data\":\"X-1Y04\"}" || move == "{\"data\":\"X-1Y-04\"}")
            {
                _direction.x -= 1;
            }

            if (move == "{\"data\":\"X0Y-14\"}" || move == "{\"data\":\"X-0Y-14\"}")
            {
                _direction.z -= 1;
            }
