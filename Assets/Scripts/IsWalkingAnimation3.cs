...
// réception mouvements/aniamtions/jumps des joueur 3
namespace walkingPlayerAnimation3
{
    public class IsWalkingAnimation3 : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (moveAnimation == "{\"data\":\"X0Y03\"}")
            {
                _direction.x = 0;
                _direction.z = 0;
            }

            if (moveAnimation == "{\"data\":\"X1Y13\"}")
            {
                _direction.x += 1;
                _direction.z += 1;
            }

            if (moveAnimation == "{\"data\":\"X-1Y-13\"}")
            {
                _direction.x -= 1;
                _direction.z -= 1;
            }

            if (moveAnimation == "{\"data\":\"X1Y03\"}" || moveAnimation == "{\"data\":\"X1Y-03\"}")
            {
                _direction.x += 1;
            }

            if (moveAnimation == "{\"data\":\"X0Y13\"}" || moveAnimation == "{\"data\":\"X-0Y3\"}")
            {
                _direction.z += 1;
            }

            if (moveAnimation == "{\"data\":\"X-1Y03\"}" || moveAnimation == "{\"data\":\"X-1Y-03\"}")
            {
                _direction.x -= 1;
            }

            if (moveAnimation == "{\"data\":\"X0Y-13\"}" || moveAnimation == "{\"data\":\"X-0Y-13\"}")
            {
                _direction.z -= 1;
            }
