...
// réception mouvements/aniamtions/jumps du joueur 2
namespace walkingPlayer2
{
    public class PlayerMovement2 : MonoBehaviour
    {
        private AfficheButton2 _testJson;
        ...
        private void Update()
        {
            ...
            if (move == "{\"data\":\"X-1Y-12\"}")
            {
                _direction.x -= 1;
                _direction.z -= 1;
            }

            if (move == "{\"data\":\"X1Y02\"}" || move == "{\"data\":\"X1Y02\"}")
            {
                _direction.x += 1;
            }

            if (move == "{\"data\":\"X0Y12\"}" || move == "{\"data\":\"X-0Y12\"}")
            {
                _direction.z += 1;
            }

            if (move == "{\"data\":\"X-1Y02\"}" || move == "{\"data\":\"X-1Y-02\"}")
            {
                _direction.x -= 1;
            }

            if (move == "{\"data\":\"X0Y-12\"}" || move == "{\"data\":\"X-0Y-12\"}")
            {
                _direction.z -= 1;
            }
